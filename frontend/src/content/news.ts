...
export const NEWS: readonly NewsArticle[] = [
  {
    ...
    summary:
      "Миниатюры показывают доказательные опоры клеток, а под лабораторией выбранного семейства расположен полный последовательный LaTeX-текст доказательства.",
    ...
    body: [
      ...
      "Для каждого семейства прямо под лабораторией показывается текст с явными корнями, выводом цветовых тождеств, восстановлением E, x, y, разбором целочисленности и ссылками на общие леммы. Все стартовые пресеты дают девять положительных попарно различных клеток и ровно заявленные пять квадратов.",
    ],
  },
  {
