...
export const NEWS: readonly NewsArticle[] = [
  {
    slug: "coordinate-model-restored",
    date: "2026-07-21",
    title: "Координаты E, x, y снова в центре интерфейса",
    summary:
      "Семейства стали пресетами универсального квадрата, а исходные инструменты управления вернулись в React SPA.",
    tags: ["frontend", "coordinates", "alpha"],
    body: [
      "Основное состояние интерфейса теперь совпадает с математической моделью проекта: любой квадрат однозначно задаётся тройкой E, x, y. Эти координаты постоянно показаны рядом с матрицей и доступны для прямого редактирования.",
      "Параметрические семейства вычисляют E, x, y, но не создают отдельное состояние. Повороты, отражение, минимизация, умножение и факторизация работают с единым текущим квадратом независимо от способа его получения.",
      "Цветовая семантика также возвращена к исходной: тёмная панель инструментов, светлая матрица, красные полные квадраты и собственные цвета классов.",
    ],
  },
  {
