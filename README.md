...
Новая React + TypeScript SPA развивается в [`frontend/`](./frontend/README.md).
Она собирается в статические файлы для nginx и пока работает параллельно с
legacy-интерфейсом `New/`. Атлас содержит все 23 орбиты `4/9` и все 23 орбиты
`5/9` относительно симметрий квадрата. Формулы с сертификатами proof-core и
перенесённые legacy-формулы имеют разные явные статусы. Для каждой маски есть
полный LaTeX-текст доказательства с явными корнями, выводом тождеств и ссылками
на общие леммы. Все стартовые пресеты невырождены и имеют ровно четыре или пять
заявленных квадратных клеток. Новости
остаются статическими и не требуют отдельной серверной части.

Интерфейс полностью доступен на русском и английском языках по устойчивым
маршрутам `/ru/...` и `/en/...`. Переключатель сохраняет текущую страницу,
параметры и якорь доказательства; переведены каталог 23+23, общая теория,
семейные доказательства, статусы полноты и новости.
