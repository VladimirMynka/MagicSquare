...
legacy-интерфейсом `New/`. Атлас содержит все 22 семейства `5/9`: восемь уже
имеют символьные сертификаты в `magic-squares-core`, а перенесённые legacy-
формулы явно отмечены как ожидающие формализации. Для каждой маски есть
полный LaTeX-текст доказательства с явными корнями, выводом тождеств и ссылками
на общие леммы. Все стартовые пресеты невырождены и имеют ровно пять заявленных
квадратных клеток. Новости
